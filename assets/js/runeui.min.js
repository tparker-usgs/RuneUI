...
$(this).on("mousemove click keypress",function(e){idleTime=0;if(viewScreenSaver){$('.tab-content').show("slow");$('.screen-saver-content').hide("slow");$('#menu-bottom').show("slow");$('#menu-top').show("slow");viewScreenSaver=0;}});}});function timerIncrement(){if($('#section-index').length&&GUI.state==='play'&&!$('#panel-sx').hasClass('active')&&!$('#panel-dx').hasClass('active')){idleTime=idleTime+1;}else{idleTime=0;}
if(idleTime>SStime){$('#section-index').addClass('disable-scrollbar-1');$('#section-index').addClass('disable-scrollbar-2');$('.tab-content').hide("slow");$('.screen-saver-content').show("slow");$('#menu-bottom').hide("slow");$('#menu-top').hide("slow");viewScreenSaver=1;}}
function checkPlayerState(){if($('#playback').hasClass('active')){$('#section-index').addClass('disable-scrollbar-1');$('#section-index').addClass('disable-scrollbar-2');}else{$('#section-index').removeClass('disable-scrollbar-1');$('#section-index').removeClass('disable-scrollbar-2');}}
...
if((vol>=0)&&(vol<=100)){setvol(vol);$('#volume').val(vol,false).trigger('update');}}
$('#countdown-display').click(function(){sendCmd('pause');});$('#total').click(function(){sendCmd('pause');});$('#random').click(function(){if($('#random').attr('data-cmd')==='pl-ashuffle-stop'){$.post('/db/?cmd=pl-ashuffle-stop','');$('#random').attr('data-cmd','random');$('#random').attr('title','Random');$('#random').removeClass('btn-primary');}});$('#panel-sx').click(function(){$('#section-index').removeClass('disable-scrollbar-1');$('#section-index').removeClass('disable-scrollbar-2');});$('#panel-dx').click(function(){$('#section-index').removeClass('disable-scrollbar-1');$('#section-index').removeClass('disable-scrollbar-2');});$('#playback').click(function(){$('#section-index').addClass('disable-scrollbar-1');$('#section-index').addClass('disable-scrollbar-2');});$('#time').knob({inline:false,change:function(value){if(GUI.state!=='stop'){window.clearInterval(GUI.currentKnob);}else{$('#time').val(0).trigger('change');}},release:function(value){onreleaseKnob(value);}});var dynVolumeKnob=$('#volume').data('dynamic');$('#volume').knob({inline:false,change:function(value){var vol=parseInt(value);if(vol>GUI.maxvol-4&&GUI.checkvol<GUI.minvol+5){$('#volume').val(0);if(dynVolumeKnob){$(document).mouseup();}
