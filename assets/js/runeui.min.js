...
$(this).on("mousemove click keypress",function(e){idleTime=0;if(viewScreenSaver){$('.tab-content').show("slow");$('.screen-saver-content').hide("slow");$('#menu-bottom').show("slow");$('#menu-top').show("slow");viewScreenSaver=0;}});}});function timerIncrement(){if($('#section-index').length&&GUI.state==='play'){idleTime=idleTime+1;}else{idleTime=0;}
if(idleTime>SStime){$('.tab-content').hide("slow");$('.screen-saver-content').show("slow");$('#menu-bottom').hide("slow");$('#menu-top').hide("slow");viewScreenSaver=1;}}
...
if(ss<10){ss='0'+ss;}
return hr+mm+'m '+ss+'s';}
function countdownRestart(startFrom){var display=$('#countdown-display').countdown('destroy');display.countdown({since:-(startFrom),compact:true,format:'MS'});var displayss=$('#countdown-display-ss').countdown('destroy');displayss.countdown({since:-(startFrom),compact:true,format:'MS'});}
...
if(isLocalHost){content+=divOpen+'<div id="home-jamendo" class="home-block'+toggleMPD+'" data-plugin="Jamendo" data-path="Jamendo"><i class="fa fa-play-circle-o"></i><h3>Jamendo<span id="home-count-jamendo"></span></h3></div>'+divClose;}else{content+=divOpen+'<div id="home-jamendo" class="home-block'+toggleMPD+'" data-plugin="Jamendo" data-path="Jamendo"><i class="fa fa-play-circle-o"></i><h3>Jamendo<span id="home-count-jamendo"></span></h3>world\'s largest platform for free music</div>'+divClose;}
content+='</div>';document.getElementById('home-blocks').innerHTML=content;loadingSpinner('db','hide');$('span','#db-currentpath').html('');}
function refreshState(){var state=GUI.state;if(state==='play'){$('#play').addClass('btn-primary');$('i','#play').removeClass('fa fa-pause').addClass('fa fa-play');$('#stop').removeClass('btn-primary');}else if(state==='pause'){$('#playlist-position span').html('Not playing');$('#playlist-position-ss span').html('Not playing');$('#play').addClass('btn-primary');$('i','#play').removeClass('fa fa-play').addClass('fa fa-pause');$('#stop').removeClass('btn-primary');}else if(state==='stop'){$('#play').removeClass('btn-primary');$('i','#play').removeClass('fa fa-pause').addClass('fa fa-play');$('#stop').addClass('btn-primary');if($('#section-index').length){$('#countdown-display').countdown('destroy');$('#countdown-display-ss').countdown('destroy');}
if(GUI.stream==='radio'){$('#total').html('<span>&infin;</span>');$('#total-ss').html('<span>&infin;</span>');}else{$('#total').html('00:00');$('#total-ss').html('00:00');}
$('#time').val(0,false).trigger('update');$('#format-bitrate').html('&nbsp;');$('#format-bitrate-ss').html('&nbsp;');$('li','#playlist-entries').removeClass('active');}
if(state!=='stop'){if(GUI.stream==='radio'){$('#total').html('<span>&infin;</span>');$('#total-ss').html('<span>&infin;</span>');}else{$('#total').html((GUI.json.time!==undefined)?timeConvert(GUI.json.time):'00:00');$('#total-ss').html((GUI.json.time!==undefined)?timeConvert(GUI.json.time):'00:00');}
var fileinfo=(GUI.json.audio_channels&&GUI.json.audio_sample_depth&&GUI.json.audio_sample_rate)?(GUI.json.audio_channels+', '+GUI.json.audio_sample_depth+' bit, '+GUI.json.audio_sample_rate+' kHz, '+GUI.json.bitrate+' kbps'):'&nbsp;';$('#format-bitrate').html(fileinfo);$('#format-bitrate-ss').html(fileinfo);$('li','#playlist-entries').removeClass('active');var current=parseInt(GUI.json.song);$('#playlist-entries').find('li').eq(current).addClass('active');}
if(GUI.json.playlistlength&&GUI.json.playlistlength!=='0'){if(GUI.json.song){$('#playlist-position span').html('Playlist position '+(parseInt(GUI.json.song)+1)+'/'+GUI.json.playlistlength);$('#playlist-position-ss span').html('Playlist position '+(parseInt(GUI.json.song)+1)+'/'+GUI.json.playlistlength);}else{$('#playlist-position span').html('Playlist position 1/'+GUI.json.playlistlength);$('#playlist-position-ss span').html('Playlist position 1/'+GUI.json.playlistlength);}}else{$('#playlist-position span').html('Empty queue, add some music!');$('#playlist-position-ss span').html('Empty queue, add some music!');}
...
if(GUI.stream!=='radio'){$('#currentartist').html((currentartist===null||currentartist===undefined||currentartist==='')?'<span class="notag">[no artist]</span>':currentartist);$('#currentartist-ss').html((currentartist===null||currentartist===undefined||currentartist==='')?'<span class="notag">[no artist]</span>':currentartist);if(currentsong===null||currentsong.length>35){$('#currentsong-ss')[0].style.fontSize="26px";}else if(currentsong.length>25){$('#currentsong-ss')[0].style.fontSize="33px";}else{$('#currentsong-ss')[0].style.fontSize="40px";}
$('#currentsong').html((currentsong===null||currentsong===undefined||currentsong==='')?'<span class="notag">[no title]</span>':currentsong);$('#currentsong-ss').html((currentsong===null||currentsong===undefined||currentsong==='')?'<span class="notag">[no title]</span>':currentsong);if(currentalbum===null||currentalbum.length>45){$('#currentalbum-ss')[0].style.fontSize="20px";}else if(currentalbum.length>30){$('#currentalbum-ss')[0].style.fontSize="24px";}else{$('#currentalbum-ss')[0].style.fontSize="30px";}
$('#currentalbum').html((currentalbum===null||currentalbum===undefined||currentalbum==='')?'<span class="notag">[no album]</span>':currentalbum);$('#currentalbum-ss').html((currentalbum===null||currentalbum===undefined||currentalbum==='')?'<span class="notag">[no album]</span>':currentalbum);}else{$('#currentartist').html((currentartist===null||currentartist===undefined||currentartist==='')?radioname:currentartist);$('#currentartist-ss').html((currentartist===null||currentartist===undefined||currentartist==='')?radioname:currentartist);$('#currentsong').html((currentsong===null||currentsong===undefined||currentsong==='')?radioname:currentsong);$('#currentsong-ss').html((currentsong===null||currentsong===undefined||currentsong==='')?radioname:currentsong);$('#currentalbum').html('<span class="notag">streaming</span>');$('#currentalbum-ss').html('<span class="notag">streaming</span>');}
if(GUI.json.repeat==='1'){$('#repeat').addClass('btn-primary');}else{$('#repeat').removeClass('btn-primary');}
...
GUI.currentsong=currentsong;var currentalbumstring=currentartist+' - '+currentalbum;if(GUI.old_state!==GUI.state||GUI.currentalbum!==currentalbumstring){GUI.old_state=GUI.state;$('#artist-bio-ss').html('');$('#artist-image-ss').css('background-image','');$('#addinfo-text-ss').html('');$('#artist-bio-overlay').html('');$('#artist-image-overlay').css('background-image','');$('#addinfo-text-overlay').html('');if(GUI.stream!=='radio'){var covercachenum=Math.floor(Math.random()*1001);$('#cover-art').css('background-image','url("/coverart/?v='+covercachenum+'")');$('#cover-art-ss').css('background-image','url("/coverart/?v='+covercachenum+'")');$.ajax({url:'/artist_info/',success:function(data){var info=jQuery.parseJSON(data);if(typeof info.artist!=='undefined'&&info.artist.bio.content!==''){$('#artist-bio-ss').html(info.artist.bio.content.substring(0,550)+' ... ');$('#artist-bio-overlay').html(info.artist.bio.summary);$('#artist-bio-full-overlay').html(info.artist.bio.content);}else{$('#artist-bio-ss').html(currentartist+', sorry, no details available ');$('#artist-bio-overlay').html(currentartist+', sorry, no details available ');$('#artist-bio-full-overlay').html(currentartist+', sorry, no details available ');}
if(typeof info.artist!=='undefined'&&info.artist.similar.artist[0].name!==''){$('#addinfo-text-ss').html('Similar Artists:<br>&nbsp;&nbsp;&nbsp;&nbsp;'+info.artist.similar.artist[0].name+'<br>&nbsp;&nbsp;&nbsp;&nbsp;'+info.artist.similar.artist[1].name+'<br>&nbsp;&nbsp;&nbsp;&nbsp;'+info.artist.similar.artist[2].name+'<br>&nbsp;&nbsp;&nbsp;&nbsp;'+info.artist.similar.artist[3].name+'<br>&nbsp;&nbsp;&nbsp;&nbsp;'+info.artist.similar.artist[4].name);$('#addinfo-text-overlay').html('Similar Artists:<br>&nbsp;&nbsp;&nbsp;&nbsp;'+info.artist.similar.artist[0].name+'<br>&nbsp;&nbsp;&nbsp;&nbsp;'+info.artist.similar.artist[1].name+'<br>&nbsp;&nbsp;&nbsp;&nbsp;'+info.artist.similar.artist[2].name+'<br>&nbsp;&nbsp;&nbsp;&nbsp;'+info.artist.similar.artist[3].name+'<br>&nbsp;&nbsp;&nbsp;&nbsp;'+info.artist.similar.artist[4].name+'<br>&nbsp;<br>&nbsp;');}else{$('#addinfo-text-ss').html('');$('#addinfo-text-overlay').html('');}
if(typeof info.artist!=='undefined'&&info.artist.image[2]!==''){$('#artist-image-ss').css('background-image','url("'+info.artist.image[2]["#text"]+'")');$('#artist-image-overlay').css('background-image','url("'+info.artist.image[2]["#text"]+'")');}else{$('#artist-image-ss').css('background-image','url("assets/img/unkown.png")');$('#artist-image-overlay').css('background-image','url("assets/img/unkown.png")');}},cache:false});}else{var covercachenum=Math.floor(Math.random()*1001);$.ajax({url:'/artist_info/',success:function(data){var info=jQuery.parseJSON(data);if(typeof info.artist!=='undefined'&&info.artist.bio.content!==''){$('#artist-bio-ss').html(info.artist.bio.content.substring(0,1000)+' ... ');$('#addinfo-text-ss').html('Similar Artists:<br>&nbsp;&nbsp;&nbsp;&nbsp;'+info.artist.similar.artist[0].name+'<br>&nbsp;&nbsp;&nbsp;&nbsp;'+info.artist.similar.artist[1].name+'<br>&nbsp;&nbsp;&nbsp;&nbsp;'+info.artist.similar.artist[2].name+'<br>&nbsp;&nbsp;&nbsp;&nbsp;'+info.artist.similar.artist[3].name+'<br>&nbsp;&nbsp;&nbsp;&nbsp;'+info.artist.similar.artist[4].name);$('#artist-image-ss').css('background-image','url("'+info.artist.image[2]["#text"]+'")');}},cache:false});$('#cover-art').css('background-image','url("assets/img/cover-radio.jpg")');$('#cover-art-ss').css('background-image','url("assets/img/cover-radio.jpg")');}}
...
function onreleaseKnob(value){if(GUI.state!=='stop'&&GUI.state!==''){if(GUI.stream!=='radio'){window.clearInterval(GUI.currentKnob);var seekto=Math.floor((value*parseInt(GUI.json.time))/1000);sendCmd('seek '+GUI.json.song+' '+seekto);$('#time').val(value);$('#countdown-display').countdown('destroy');$('#countdown-display-ss').countdown('destroy');$('#countdown-display').countdown({since:-seekto,compact:true,format:'MS'});$('#countdown-display-ss').countdown({since:-seekto,compact:true,format:'MS'});}else{$('#time').val(0).trigger('change');}}}
function commandButton(el){var dataCmd=el.data('cmd');var cmd;if(dataCmd==='stop'){el.addClass('btn-primary');$('#play').removeClass('btn-primary');if($('#section-index').length){refreshTimer(0,0,'stop');window.clearInterval(GUI.currentKnob);$('.playlist').find('li').removeClass('active');$('#total').html('00:00');$('#total-ss').html('00:00');}}
else if(dataCmd==='play'){var state=GUI.state;if(state==='play'){cmd='pause';if($('#section-index').length){$('#countdown-display').countdown('pause');$('#countdown-display-ss').countdown('pause');}}else if(state==='pause'){cmd='play';if($('#section-index').length){$('#countdown-display').countdown('resume');$('#countdown-display-ss').countdown('resume');}}else if(state==='stop'){cmd='play';if($('#section-index').length){$('#countdown-display').countdown({since:0,compact:true,format:'MS'});$('#countdown-display-ss').countdown({since:0,compact:true,format:'MS'});}}
window.clearInterval(GUI.currentKnob);sendCmd(cmd);return;}
else if(dataCmd==='previous'||dataCmd==='next'){if($('#section-index').length){$('#countdown-display').countdown('pause');$('#countdown-display-ss').countdown('pause');window.clearInterval(GUI.currentKnob);}}
...
if((vol>=0)&&(vol<=100)){setvol(vol);$('#volume').val(vol,false).trigger('update');}}
$('#countdown-display').click(function(){sendCmd('pause');});$('#total').click(function(){sendCmd('pause');});$('#random').click(function(){if($('#random').attr('data-cmd')==='pl-ashuffle-stop'){$.post('/db/?cmd=pl-ashuffle-stop','');$('#random').attr('data-cmd','random');$('#random').attr('title','Random');$('#random').removeClass('btn-primary');}});$('#time').knob({inline:false,change:function(value){if(GUI.state!=='stop'){window.clearInterval(GUI.currentKnob);}else{$('#time').val(0).trigger('change');}},release:function(value){onreleaseKnob(value);}});var dynVolumeKnob=$('#volume').data('dynamic');$('#volume').knob({inline:false,change:function(value){var vol=parseInt(value);if(vol>GUI.maxvol-4&&GUI.checkvol<GUI.minvol+5){$('#volume').val(0);if(dynVolumeKnob){$(document).mouseup();}
